...
function DMoneyFrame_OnLoad()
	...
	DMoneyFrame_SetType("PLAYER");
end

function DSmallMoneyFrame_OnLoad()
	this:RegisterEvent("PLAYER_MONEY");
	this:RegisterEvent("PLAYER_TRADE_MONEY");
	this:RegisterEvent("TRADE_MONEY_CHANGED");
	this:RegisterEvent("SEND_MAIL_MONEY_CHANGED");
...
function DMoneyFrame_SetType(type)
	local info = DMoneyTypeInfo[type];
	if ( not info ) then
...
function DSetMoneyFrameColor(frameName, r, g, b)
	-- Color the coin amount font strings directly (named "<button>Text"). The default
	-- coin font (NumberFontNormalLarge) is white with a black drop shadow, which is
	-- illegible on the parchment, so recolor dark and drop the shadow.
	local suffixes = { "GoldButton", "SilverButton", "CopperButton" };
	for i = 1, 3 do
		local fontString = getglobal(frameName .. suffixes[i] .. "Text");
		if (fontString) then
